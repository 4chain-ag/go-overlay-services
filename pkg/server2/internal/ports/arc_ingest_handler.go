...
func (h *ArcIngestHandler) HandleArcIngest(c *fiber.Ctx) error {
	...
	// Check for specific error types first to ensure correct status codes
	if err != nil {
		if errors.Is(err, app.ErrMerkleProofProcessingTimeout) {
			return c.Status(fiber.StatusGatewayTimeout).JSON(openapi.Error{
				Message: app.ErrMerkleProofProcessingTimeout.Error(),
			})
		}

		if errors.Is(err, app.ErrMerkleProofProcessingCanceled) {
			return c.Status(fiber.StatusRequestTimeout).JSON(openapi.Error{
				Message: app.ErrMerkleProofProcessingCanceled.Error(),
			})
		}

		if errors.Is(err, app.ErrMerkleProofProcessingFailed) {
			return c.Status(fiber.StatusInternalServerError).JSON(openapi.Error{
				Message: app.ErrMerkleProofProcessingFailed.Error(),
			})
		}

		if errors.Is(err, app.ErrInvalidTxIDFormat) || errors.Is(err, app.ErrInvalidTxIDLength) || errors.Is(err, app.ErrInvalidMerklePathFormat) {
