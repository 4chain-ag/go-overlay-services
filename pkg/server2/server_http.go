...
func WithEngine(provider engine.OverlayEngineProvider) ServerOption {
	return func(s *ServerHTTP) {
		...
		s.topicManagerDocumentationHandler = ports.NewTopicManagerDocumentationHandler(provider)
	}
}
...
func (s *ServerHTTP) registerRoutes() {
	...
	v1.Get("/getDocumentationForTopicManager", s.topicManagerDocumentationHandler.GetDocumentation)
